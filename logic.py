...
# ── Location of the prppi tool on this machine ───────────────────────────
# main.py lives inside C:\prppi-main\prppi, and it imports itself as a
# package (e.g. "from prppi import sort_defined_interface"), so Python
# needs C:\prppi-main (the parent folder) on its import path (PYTHONPATH).
PRPPI_MAIN = r"C:\prppi-main\prppi\main.py"
PRPPI_ROOT = r"C:\prppi-main"
...
def run_prppi(pdb_path, cutoff=5.0, groups=None):
    ...
    cmd = [
        sys.executable, PRPPI_MAIN, pdb_path,
        "--side_1", str(side_1),
        "--side_2", str(side_2),
        "--cutoff", str(cutoff),
    ]
    ...
    # Make sure the prppi package (C:\prppi-main) is importable by the
    # subprocess, since main.py does "from prppi import ..."
    env = os.environ.copy()
    env["PYTHONPATH"] = PRPPI_ROOT + os.pathsep + env.get("PYTHONPATH", "")
    ...
    # (main.py tries to open the result with the 'nano' editor at the end,
    # which doesn't exist on Windows, so this specific failure is expected.)
